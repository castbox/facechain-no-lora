...
def post_process_tags(result_list, score_gender, score_age):
    """ Process the tags generated by DeepDanbooru

    Keeps the essential information like skin, hair style, etc. and removes the rest.

    Args:
        result_list (list): List of tag list (each image has one list) generated by DeepDanbooru
        score_gender: 0 if classified as a man
        score_age: the score used to classify the age

    Examples of elements in the result list:  
    ['1girl', 'black_hair', 'closed_mouth', 'forehead', 'lips', 'long_hair', 'looking_at_viewer', 'nose', 'portrait', 'smile', 'solo', 'transparent_background']
    ['1girl', 'black_hair', 'blue_eyes', 'face', 'forehead', 'grin', 'long_hair', 'looking_at_viewer', 'mole', 'mole_under_eye', 'nose', 'portrait', 'smile', 'solo', 'teeth', 'transparent_background']
    ['1girl', 'black_hair', 'face', 'long_hair', 'looking_at_viewer', 'nose', 'portrait', 'smile', 'solo', 'teeth', 'transparent_background']
    ...
    result_list_new = []
    for result in result_list:
        print(f"Debug Info: the tag is: {result}")
        result_new = []
        result_new.extend(tag_a_g)
        ## don't include other infos for lora training
        #for tag in result:
        ...
        #    # import pdb;pdb.set_trace()
        ## result_new.append('slim body')   
        # include information related to skin, hair style, facial expression, etc.
        for tag in result:
            if "skin" in tag or "hair" in tag or "smile" in tag or "grin" in tag:
                result_new.append(tag)
    ...
    def __call__(self, imdir):
        ...
        for imname in imlist:
            try:
                ...
                print(f"Debug Info: the tags generated are: {result}")
        ...
        result_list = post_process_tags(result_list, score_gender, score_age)
        self.model.stop()
        try:
            os.remove(tmp_path)
        except OSError as e:
